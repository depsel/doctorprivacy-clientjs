...
import { LeakipediaRiskLevel, LeakipediaSource, riskLevelToString } from '..';

const HIBP_DATA_CLASSES: {[key: string]: string|string[]} = {
...
function dataclassesToDataCategoriesText(dataClasses: string[]): string[] {
  let result: Set<string> = new Set<string>();

  dataClasses.forEach((v)=>{
    let dataClass: string|string[] = HIBP_DATA_CLASSES[v]; 
...
function estimateRiskLevel(categories: string[]): LeakipediaRiskLevel {
  let maxRiskLevel: LeakipediaRiskLevel = LeakipediaRiskLevel.Low;
  ...
  categories.forEach((v)=>{
    if (maxRiskLevel < LeakipediaRiskLevel.Medium && mediumCategories.includes(v)) maxRiskLevel = LeakipediaRiskLevel.Medium;
    if (maxRiskLevel < LeakipediaRiskLevel.High && highCategories.includes(v)) maxRiskLevel = LeakipediaRiskLevel.High;
    if (maxRiskLevel < LeakipediaRiskLevel.Critical && criticalCategories.includes(v)) maxRiskLevel = LeakipediaRiskLevel.Critical;
  });

  return maxRiskLevel;
}

export function hIBPBreachToLeakipediaSource(breach: HIBPBreach): LeakipediaSource {
  const DATE_FORMAT = 'YYYYMMDD';
  const breachDate: string = moment(breach.BreachDate).format(DATE_FORMAT);
  const publicationDate: string = moment(breach.AddedDate).format(DATE_FORMAT);
  const entryDate: string = moment().format(DATE_FORMAT);

  const dataCategories: string[] = dataclassesToDataCategoriesText(breach.DataClasses);

  const riskLevel: LeakipediaRiskLevel = estimateRiskLevel(dataCategories);
  ...
  return {
    ...
    risk: riskLevel,
    riskText: riskLevelToString(riskLevel),
